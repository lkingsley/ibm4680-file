...
def hex_to_int(hex_string):
    # Works on python 2.7.18
    if hasattr(hex_string, 'encode'):
...
def is_keyed(file_name):
    ...
    with open(file_name, 'rb') as f:
        ...
        year = hex_to_int(control_block[30]) | \
            (hex_to_int(control_block[31]) << 8)
        month = hex_to_int(control_block[32])
        day = hex_to_int(control_block[33])
        kfile.creation_date = '{}/{}/{}'.format(year, month, day)

        kfile.blocks_qty = hex_to_int(control_block[42]) | \
            (hex_to_int(control_block[43]) << 8) | \
            (hex_to_int(control_block[44]) << 16) | \
            (hex_to_int(control_block[45]) << 24)
        kfile.key_length = hex_to_int(control_block[54]) | \
            (hex_to_int(control_block[55]) << 8)
        kfile.record_size = hex_to_int(control_block[46]) | \
            (hex_to_int(control_block[47]) << 8)
